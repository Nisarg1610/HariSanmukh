...
useEffect(() => {
  ...
  const handleSetupProfile = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Check if email exists as a household_member — link them
      ...
      if (existingMember) {
        ...
        await supabase
          .from('household_members')
          .update({ linked_user_id: user.id, first_name: firstName.trim() })
          .eq('id', existingMember.id);
      } else {
        // Create new household (this person becomes admin)
      ...
      const { error: uErr } = await supabase.from('users').insert({
        ...
        role: existingMember ? 'user' : 'admin',
        status: 'active',
      });
      if (uErr) throw uErr;
    ...
    return (
      <main className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-slate-900 dark:to-slate-800 flex items-center justify-center px-4">
        <div className="w-full max-w-md">
          <div className="text-center mb-10">
            <h1 className="text-5xl font-bold text-gray-900 dark:text-white mb-2">HariSanmukh</h1>
            <p className="text-gray-600 dark:text-gray-300">Welcome! Let's set up your profile.</p>
          </div>
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl p-8">
            {error && (
              <div className="mb-4 p-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg">
            ...
            <form onSubmit={handleSetupProfile} className="space-y-4">
              <div>
                ...
                <div className="flex gap-2 items-center">
                  ...
                  />
                  <span className="text-gray-500 dark:text-gray-400 font-medium">Bhai</span>
                </div>
              </div>
              <button
                type="submit"
            ...
            <button onClick={handleLogout} className="w-full mt-3 text-sm text-gray-500 hover:text-gray-700 dark:hover:text-gray-300">
  ...
  return (
    <main className="min-h-screen bg-white dark:bg-slate-950 pb-28">
      ...
      <div className="max-w-2xl mx-auto px-4 py-8">
        ...
        <p className="text-gray-500 dark:text-gray-400 mb-8 text-sm">{user.email}</p>

        <div className="grid grid-cols-3 gap-3 mb-8">
          {[
            { label: 'My Sevas', value: '—', color: 'blue' },
            { label: 'Completed', value: '—', color: 'green' },
            { label: 'Members', value: '—', color: 'purple' },
          ].map(({ label, value, color }) => (
            <div key={label} className={`bg-${color}-50 dark:bg-${color}-900/20 border border-${color}-200 dark:border-${color}-800 rounded-2xl p-4`}>
        ...
        <div className="bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-200 dark:border-indigo-800 rounded-2xl p-6">
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400 mb-3">Use the navigation below to manage sevas, grocery, laundry, and members.</p>
          {dbUser.role === 'admin' && (
            <span className="inline-block bg-blue-600 text-white text-xs font-semibold px-3 py-1 rounded-full">Admin</span>
          )}
        </div>
      </div>
